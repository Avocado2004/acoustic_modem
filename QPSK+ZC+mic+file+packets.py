...
if __name__ == "__main__":
  ...
  if op == "T":
    ...
    # ideal ZC OFDM symbol (time domain without CP) is at preamble_td[Ncp : Ncp+Nfft]
    ideal_zc = preamble_td[Ncp : Ncp + Nfft]
    # received ZC1 and ZC2 useful parts
    try:
      rx_zc1 = rx[sync_idx + Ncp : sync_idx + Ncp + Nfft]
      rx_zc2 = rx[sync_idx + SYMBOL_LEN + Ncp : sync_idx + SYMBOL_LEN + Ncp + Nfft]
      # compute phase difference between received repeats weighted by ideal pattern
      # use vector inner product for robust estimate
      cross = np.vdot(rx_zc1, rx_zc2)  # vdot(conj(a), b)
      # angle of cross gives overall phase rotation between repeats
      delta_phi = np.angle(cross)
      # time between starts of the two ZC OFDM symbols is SYMBOL_LEN samples
      T_between = SYMBOL_LEN / float(fs)
      f_err = delta_phi / (2.0 * np.pi * T_between)  # Hz
      print(f"[RX-DBG] FOE estimate: delta_phi={delta_phi:.6f} rad, f_err={f_err:.3f} Hz")
    except Exception as e:
      f_err = 0.0
      print("[RX-DBG] FOE estimation failed, assuming 0 Hz:", e)

    # Estimate channel Hk using pilot symbols that follow ZC pair (pilot1 at offset 2*SYMBOL_LEN)
    try:
      pilot1_start = sync_idx + 2*SYMBOL_LEN
      rx_pre1 = rx[pilot1_start + Ncp : pilot1_start + Ncp + Nfft]
      pilot2_start = pilot1_start + SYMBOL_LEN
      rx_pre2 = rx[pilot2_start + Ncp : pilot2_start + Ncp + Nfft]
      S_ref = np.fft.fft(preamble_td[2*SYMBOL_LEN + Ncp : 2*SYMBOL_LEN + Ncp + Nfft])  # ideal pilot
      # compensate FOE on these pilot time samples before FFT: multiply by exp(-j*2*pi*f_err * t)
      # compute time offsets for samples (relative to sync_idx)
      t1_offset = (pilot1_start) / float(fs)
      t2_offset = (pilot2_start) / float(fs)
      time_vec = np.arange(Nfft) / float(fs)
      rx_pre1_corr = rx_pre1 * np.exp(-1j * 2.0 * np.pi * f_err * (t1_offset + time_vec))
      rx_pre2_corr = rx_pre2 * np.exp(-1j * 2.0 * np.pi * f_err * (t2_offset + time_vec))
      R1, R2 = np.fft.fft(rx_pre1_corr), np.fft.fft(rx_pre2_corr)
      ...
      print("[RX-ERR] pilot-based Hk estimation failed:", e)
      # fallback simple estimate using preamble_td ideal_sym (first pilot-like symbol fallback)
      try:
        ideal_sym = preamble_td[Ncp : Ncp + Nfft]
        rx_pre1_fb = rx[sync_idx + Ncp : sync_idx + Ncp + Nfft]
        rx_pre2_fb = rx[sync_idx + SYMBOL_LEN + Ncp : sync_idx + SYMBOL_LEN + Ncp + Nfft]
        R1_fb, R2_fb = np.fft.fft(rx_pre1_fb), np.fft.fft(rx_pre2_fb)
        S_ref_fb = np.fft.fft(ideal_sym)
        Hk = ((R1_fb[subc_inds]/S_ref_fb[subc_inds]) + (R2_fb[subc_inds]/S_ref_fb[subc_inds]))/2
        Hk_mag_s = np.clip(medfilt(np.abs(Hk), 5), 1/2.0, None)
        Hk_smooth = Hk_mag_s * np.exp(1j*np.angle(Hk))
      except Exception as e2:
        print("[RX-ERR] fallback Hk failed:", e2)
        Hk_smooth = np.ones(len(subc_inds), dtype=complex)

    # Now process payload frames with FOE compensation per-frame
    frames = segment.reshape(nblk, SYMBOL_LEN)
    rx_subc_list = []
    # For each frame, take its useful part, apply FOE compensation depending on absolute time, FFT and divide by Hk_smooth
    for idx, frm in enumerate(frames):
      # absolute sample index of start of this frame in rx
      frame_start_abs = start + idx * SYMBOL_LEN
      t_frame_start = frame_start_abs / float(fs)
      # useful samples are frm[Ncp:]
      useful = frm[Ncp:]
      # time vector for these Nfft samples
      tv = t_frame_start + np.arange(Nfft) / float(fs)
      # apply FOE compensation: multiply by exp(-j*2*pi*f_err * t)
      useful_corr = useful * np.exp(-1j * 2.0 * np.pi * f_err * tv)
      # FFT and equalize
      try:
        F = np.fft.fft(useful_corr)
        subc = F[subc_inds] / Hk_smooth
      except Exception:
        subc = np.zeros(Nsub, dtype=complex)
      rx_subc_list.append(subc)
    rx_subc = np.concatenate(rx_subc_list)
