...
# Количество пилотных символов перед преамбулой для настройки эквалайзера и AGC
# Пилотный символ: все поднесущие = (1+1j)/√2 (как в преамбуле)
PREAMBLE_PILOT_SYMBOLS = 16  # количество пилотных OFDM символов перед преамбулой

# Интервал вставки пилотных символов внутрь потока данных
# Пилот вставляется после каждых N символов данных для непрерывной подстройки эквалайзера
# Отключено: inline-пилоты убраны, используются только пилоты перед преамбулой (PREAMBLE_PILOT_SYMBOLS)
PILOT_INTERVAL = 0  # 0 = пилоты внутрь потока данных не вставляются

# Устаревшие параметры warmup (сохранены для обратной совместимости)
WARMUP_SYMBOLS = 0   # отключено, используются пилотные символы
WARMUP_LEVEL = 0.3   # уровень шума для warmup-символов (не используется)
...
SYMBOL_TARGET_RMS = 1.0  # Целевой RMS для OFDM символа (передача и прием) — увеличен для лучшей работы эквалайзера
AGC_ALPHA = 0.3            # экспоненциальный коэффициент для скользящего оценщика — увеличен для быстрой адаптации

# Включение/отключение адаптивного эквалайзера (AdaptiveEqualizer)
# True — эквалайзер активен (выравнивает частотную характеристику канала)
# False — эквалайзер отключён (символы пропускаются без выравнивания)
EQUALIZER_ENABLED = False  # По умолчанию отключён
# debug: вывод AGC по каждому OFDM-блоку (False в нормальной работе)
AGC_DEBUG = True  # Включено для отладки: вывод AGC по каждому OFDM-блоку
