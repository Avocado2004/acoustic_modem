...
# Изменения:
# - В open_settings() метки фиксированной ширины (width=160) и строки высоты 48 — текст не налезает
# - Кнопки в главном окне выше: height=56
# - Добавлена кнопка Load WAV рядом с Save WAV — выбирает кодированный WAV-файл и пытается расшифровать его с помощью функций модема
# - Остальной функционал сохранён (Receive очищает поле, диалоги с Filename сверху и т.д.)
...
class OfdmRoot(BoxLayout):
    ...
    def _build_controls(self):
        # increased button height to 56
        ctr = BoxLayout(orientation='horizontal', size_hint_y=None, height=56, spacing=6)
        btn_generate = Button(text='Generate', size_hint_x=1/4)
        btn_generate.bind(on_release=lambda b: self.on_generate_audio())
        ctr.add_widget(btn_generate)

        btn_save_audio = Button(text='Save WAV', size_hint_x=1/4)
        btn_save_audio.bind(on_release=lambda b: self.on_save_audio())
        ctr.add_widget(btn_save_audio)

        # New Load WAV button: открывает диалог выбора WAV с закодированными данными и пытается расшифровать
        btn_load_wav = Button(text='Load WAV', size_hint_x=1/4)
        btn_load_wav.bind(on_release=lambda b: self.on_load_wav())
        ctr.add_widget(btn_load_wav)

        self.btn_transmit = Button(text='Play/Stop', size_hint_x=1/4)
    ...
    def _build_diag_recv(self):
        ...
        # increased bottom buttons height to 56
        bottom = BoxLayout(orientation='horizontal', size_hint_y=None, height=56, spacing=6)
    ...
    def open_filechooser(self, *a):
        content = BoxLayout(orientation='vertical', spacing=6, padding=(6,6,6,6))
        # Filename row on top, larger height so text is not cropped
        name_row = BoxLayout(size_hint_y=None, height=48, spacing=6)
        lbl = Label(text='Filename', size_hint_x=None, width=150, valign='middle')
        lbl.text_size = (lbl.width, None)
        name_row.add_widget(lbl)
        name_input = TextInput(text='', multiline=False, size_hint_x=1, height=44, size_hint_y=None)
        ...
        popup = Popup(title='Select file', content=content, size_hint=(0.9, 0.9), auto_dismiss=False)
        def _ok(b):
    ...
    def open_settings(self):
        grid_root = BoxLayout(orientation='vertical', spacing=6, padding=(6,6,6,6))
        sv = ScrollView(size_hint=(1, 1))
        grid = GridLayout(cols=2, spacing=6, size_hint_y=None)
        row_h = 48  # increased row height for readability
        keys = list(self.settings.keys())
        grid.bind(minimum_height=grid.setter('height'))
        text_inputs = {}
        for k in keys:
            lbl = Label(text=str(k), size_hint_y=None, height=row_h, halign='left', valign='middle')
            # fixed width so label text doesn't overlap the input field
        ...
        apply_btn.size_hint_x = 0.5; cancel_btn.size_hint_x = 0.5
    ...
    def on_save_audio(self):
        ...
        fn_input = TextInput(text='ofdm_tx.wav', multiline=False, size_hint_x=1, height=44, size_hint_y=None)
        ...
        btns.add_widget(ok); btns.add_widget(cancel)
        content.add_widget(btns)
        popup = Popup(title='Save WAV', content=content, size_hint=(0.9,0.9), auto_dismiss=False)
    ...
    def on_load_wav(self):
        # Open file chooser for selecting a coded WAV file
        content = BoxLayout(orientation='vertical', spacing=6, padding=(6,6,6,6))
        top_row = BoxLayout(size_hint_y=None, height=48, spacing=6)
        lbl = Label(text='WAV file', size_hint_x=None, width=150, valign='middle')
        lbl.text_size = (lbl.width, None)
        top_row.add_widget(lbl)
        fn_input = TextInput(text='', multiline=False, size_hint_x=1, height=44, size_hint_y=None)
        top_row.add_widget(fn_input)
        content.add_widget(top_row)

        fc = FileChooserListView(path='.', size_hint=(1, 1), filters=['*.wav', '*.WAV'])
        content.add_widget(fc)
        ...
        btns.add_widget(ok); btns.add_widget(cancel)
        content.add_widget(btns)
        popup = Popup(title='Load coded WAV and decode', content=content, size_hint=(0.9,0.9), auto_dismiss=False)
        ...
        def _do_load(b):
            ...
            if fc.selection:
                ...
                folder = fc.path
                if not name:
                    self.append_diag(fmt_log_line("Load WAV: no file selected"))
                    popup.dismiss()
                    return
                path = os.path.join(folder, name)
            if not os.path.isfile(path):
                self.append_diag(fmt_log_line(f"Load WAV: file not found: {path}"))
                popup.dismiss()
                return

            popup.dismiss()
            # run decoding in background thread
            threading.Thread(target=lambda: self._background_load_and_decode(path), daemon=True).start()

        ok.bind(on_release=_do_load)
        cancel.bind(on_release=lambda b: popup.dismiss())
        popup.open()

    def _background_load_and_decode(self, path):
        try:
            self.append_diag(fmt_log_line(f"Load WAV: selected {path}, preparing modem for file decode..."))
            if _modem is None:
                self.append_diag(fmt_log_line("Load WAV: modem module not loaded"))
                return

            # Apply settings into modem globals where applicable
            try:
                for k, v in self.settings.items():
                    if hasattr(_modem, k):
                        setattr(_modem, k, v)
            except Exception as e:
                self.append_diag(fmt_log_line(f"Load WAV: failed to apply settings to modem: {e}"))
            ...
            if not hasattr(_modem, "preamble_td") or _modem.preamble_td is None:
                if hasattr(_modem, "build_preamble"):
                    try:
                        ...
                        self.append_diag(fmt_log_line("Load WAV: preamble_td built in modem"))
                    except Exception as e:
                        self.append_diag(fmt_log_line(f"Load WAV: build_preamble() failed: {e}"))
                else:
                    self.append_diag(fmt_log_line("Load WAV: modem.build_preamble() not found"))

            if hasattr(_modem, "init_phases"):
                try:
                    _modem.init_phases()
                    self.append_diag(fmt_log_line("Load WAV: init_phases() called"))
                except Exception as e:
                    self.append_diag(fmt_log_line(f"Load WAV: init_phases() failed: {e}"))

            # Preferred: call an exported modem function that processes a WAV file directly
            for func_name in ("receive_from_wav", "file_receive", "process_wav_file", "receive_wav"):
                fn = gattr(_modem, func_name, None)
                if fn is not None and callable(fn):
                    try:
                        self.append_diag(fmt_log_line(f"Load WAV: calling modem.{func_name}()"))
                        fn(path)
                        self.append_diag(fmt_log_line(f"Load WAV: modem.{func_name}() returned"))
                        return
                    except Exception as e:
                        self.append_diag(fmt_log_line(f"Load WAV: modem.{func_name}() failed: {e}"))
                        # try next option

            # If no single-function API exists, attempt to set _modem.rx and invoke
            # a reasonable exported entrypoint (live_receive_and_process handles microphone only)
            # So only attempt if modem provides a dedicated file-processing function; otherwise stop.
            self.append_diag(fmt_log_line("Load WAV: no direct file-receive function found in modem"))
            self.append_diag(fmt_log_line("Load WAV: please add an exported function in modem like receive_from_wav(path) that performs file-based decode"))
        except Exception as e:
            self.append_diag(fmt_log_line("Load WAV error: " + str(e)))
    ...
    def on_save_received(self):
        ...
        if mode == "Text":
            ...
            if not content:
                ...
                return
            root = BoxLayout(orientation='vertical', spacing=6, padding=(6,6,6,6))
            top_row = BoxLayout(size_hint_y=None, height=48, spacing=6)
            lbl = Label(text='Filename', size_hint_x=None, width=150, valign='middle')
            lbl.text_size = (lbl.width, None)
            top_row.add_widget(lbl)
            name_input = TextInput(text='received.txt', multiline=False, size_hint_x=1, height=44, size_hint_y=None)
            ...
            popup = Popup(title='Save received text as', content=root, size_hint=(0.9,0.9), auto_dismiss=False)
            def _ok(b):
                sel_dir = fc.path
                if fc.selection:
                    sel = fc.selection[0]
        ...
        if self.last_rx_saved_path and os.path.exists(self.last_rx_saved_path):
            ...
            name_input = TextInput(text=default_name, multiline=False, size_hint_x=1, height=44, size_hint_y=None)
            top_row.add_widget(name_input)
            root.add_widget(top_row)
            ...
            btns.add_widget(ok); btns.add_widget(cancel)
            root.add_widget(btns)
            popup = Popup(title='Save received file as', content=root, size_hint=(0.9,0.9), auto_dismiss=False)
            def _ok(b):
        ...
        lbl.text_size = (lbl.width, None)
        top_row.add_widget(lbl)
        name_input = TextInput(text='received.bin', multiline=False, size_hint_x=1, height=44, size_hint_y=None)
        top_row.add_widget(name_input)
        root.add_widget(top_row)
