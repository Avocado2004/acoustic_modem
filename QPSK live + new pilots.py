...
# how many first packets to print verbose Hk diagnostics
LOG_HK_FIRST_PACKETS = 20
...
def build_header(mode: bytes, data_len: int, filename_bytes: bytes = b'', packet_no: int = 0, version: int = 0, packet_blocks: int = DEFAULT_PACKET_BLOCKS) -> bytes:
    ...
    # old: flags = 0x80 | (tx_type & 0x03)
    # new: set top 4 bits to 1 (0xF0) and keep tx_type low bits
...
def parse_header(header64: bytes):
    ...
    b0 = header64[0]
    # old: is_transmission = bool(b0 & 0x80)
...
# -----------------------
# остальные вспомогательные функции (unchanged по смыслу)
# -----------------------
...
# -----------------------
# decode_packet_at_candidate (оставляем предыдущую реализацию с фазовой компенсацией)
# -----------------------
...
def decode_packet_at_candidate(pref_abs, packet_blocks_expected, packet_idx=0, bytes_before_packet=0, expected_total=0):
    ...
    # prepare ideal ZC freq-domain placement for phase estimation
    try:
    ...
    for cand in combined:
        try:
            # --- ZC kept for coarse common phase, pilots used for FOE and channel estimates ---
            # extract ZC halves (remove CP) and FFT
            rx_zc1 = rx[cand + Ncp : cand + Ncp + Nfft]
            rx_zc2 = rx[cand + SYMBOL_LEN + Ncp : cand + SYMBOL_LEN + Ncp + Nfft]
            try:
                Rzc1 = np.fft.fft(rx_zc1)
                Rzc2 = np.fft.fft(rx_zc2)
            except Exception:
                continue

            # coarse common phase from ZC (if available) — use for phase alignment only
            phi_zc = 0.0
            if S_zc_fd is not None:
                try:
                    phi_zc = np.angle(np.vdot(S_zc_fd, Rzc1))
                except Exception:
                    phi_zc = 0.0

            # pilots: always remove CP before FFT, then use pilot cross to estimate FOE (delta phase)
            ...
            # temporary FFTs of raw pilots (will correct FOE using pilot cross)
            try:
                R_p1_raw = np.fft.fft(rx_pre1)
                R_p2_raw = np.fft.fft(rx_pre2)
            except Exception:
                continue

            # per-packet FOE estimate from pilot cross (robust median of per-subcarrier delta)
            cross_p = R_p2_raw[subc_inds] * np.conj(R_p1_raw[subc_inds])
            delta_phi_per_subc = np.angle(cross_p)
            # unwrap and take median to be robust
            delta_phi_med = np.median(np.unwrap(delta_phi_per_subc))
            T_between = SYMBOL_LEN / float(fs)
            f_err_loc = delta_phi_med / (2.0 * np.pi * T_between)

            # now correct FOE on pilot time-domain symbols and recompute FFTs
            ...
            # remove coarse ZC common phase from pilots
            if phi_zc != 0.0:
                R1t *= np.exp(-1j * phi_zc)
                R2t *= np.exp(-1j * phi_zc)

            # reference pilot spectrum (from preamble)
            S_ref = np.fft.fft(preamble_td[2*SYMBOL_LEN + Ncp : 2*SYMBOL_LEN + Ncp + Nfft])

            # compute per-subcarrier channel estimates from the two pilots
            H_p1 = np.zeros(Nsub, dtype=complex)
            H_p2 = np.zeros(Nsub, dtype=complex)
            try:
                ref_slice = S_ref[subc_inds]
                valid_ref = np.abs(ref_slice) > 0
                if np.any(valid_ref):
                    H_p1[valid_ref] = R1t[subc_inds][valid_ref] / ref_slice[valid_ref]
                    H_p2[valid_ref] = R2t[subc_inds][valid_ref] / ref_slice[valid_ref]
                else:
                    continue
            except Exception:
                continue

            # robust combine: per-subcarrier median of available pilot estimates (ZC kept only for phase)
            H_stack = np.vstack([H_p1, H_p2])
            Hk_est = np.median(H_stack, axis=0)

            # smoothing and flooring/capping for stability
            Hk_mag = medfilt(np.abs(Hk_est), 5)
            floor_val = max(5e-3, np.percentile(Hk_mag, 5) * 0.2)
            cap_val = max(np.percentile(Hk_mag, 95) * 5.0, 1e2)
            Hk_mag = np.clip(Hk_mag, floor_val, cap_val)
        ...
        for idxf, fr in enumerate(frames):
            ...
            try:
                ...
                # safe division: avoid very small denominators by flooring magnitude while preserving phase
                Hk_safe = Hk_s.copy()
                Hk_abs = np.abs(Hk_safe)
                # eps_floor based on lower-percentile magnitude
                eps_floor = max(1e-3, np.percentile(Hk_abs, 10) * 0.2)
                small_idx = Hk_abs < eps_floor
                if np.any(small_idx):
                    Hk_safe[small_idx] = eps_floor * np.exp(1j * np.angle(Hk_safe[small_idx]))
                subc = F[subc_inds] / Hk_safe
            except Exception:
        ...
        try:
            ...
            continue
        # apply phi_est compensation to received symbols (remove residual common phase)
        try:
            if 'phi_est' in locals() and phi_est != 0.0:
                rx_subc_pkt = rx_subc_pkt * np.exp(-1j * phi_est)
        except Exception:
            pass
        # normalize per-packet to reduce influence of outliers before demapping
        if np.any(~np.isfinite(rx_subc_pkt)):
            continue
        mag_med = np.median(np.abs(rx_subc_pkt))
        if not np.isfinite(mag_med) or mag_med <= 1e-6:
            mag_med = 1.0
        # soft clip large samples relative to median to limit rare huge outliers
        clip_level = max(mag_med * 10.0, 1.0)
        rx_subc_pkt = np.where(np.abs(rx_subc_pkt) > clip_level,
                               clip_level * np.exp(1j * np.angle(rx_subc_pkt)),
                               rx_subc_pkt)
        rx_syms_pkt = rx_subc_pkt / mag_med
        if subc_phases is not None and np.any(subc_phases != 0):
            rx_syms_pkt = rx_syms_pkt * np.tile(np.exp(-1j * subc_phases), packet_blocks_expected)
        # skip corrupted symbol arrays
        if np.any(~np.isfinite(rx_syms_pkt)):
            continue
        ...
        for ci in range(n_cw):
            ...
            try:
                ...
                if do_print:
                    ...
                    if packet_idx == 0:
                        print(f"[RX-DBG-PKT] pkt{packet_idx} cw_idx={ci} RS_FAIL global_offset={global_offset} cw_in={bts.hex()} exc={e}")
                    else:
                        print(f"[RX-DBG-PKT] pkt{packet_idx} cw_idx={ci} RS_FAIL global_offset={global_offset}")
            decoded_blocks.append(msg)

        # If this is first packet and header looks invalid try alternate demaps
        if packet_idx == 0:
            assembled_try = b"".join(decoded_blocks)
            # changed rule: Transmission header expected when high nibble != 0
            if len(assembled_try) >= 1 and not ((assembled_try[0] & 0xF0) != 0):
                ...
                if alt_decoded is not None and alt_rs_ok > rs_ok:
                    header_len_bytes = min(len(alt_decoded), len(assembled_try))
                    assembled_try = alt_decoded + assembled_try[header_len_bytes:]
                    rs_ok = alt_rs_ok
                    decoded_blocks = [assembled_try[i:i+RS_DATA_BYTES] for i in range(0, len(assembled_try), RS_DATA_BYTES)]
                    if globals()['_rs_fail_prints_count'] < 5:
                        print("[RX-DBG-PKT] pkt0: header recovered using alternate demap (improved RS_OK)")
...
# -----------------------
# Live receive and main flow (keeps previous behavior, but header checks use high nibble)
# -----------------------
            ...
            try:
                ...
                if 'rx_syms_list' in globals() and isinstance(globals()['rx_syms_list'], list) and len(globals()['rx_syms_list']) > 0:
                    ...
                    if len(concat_list) > 0:
                        ...
                        figs_shown += 1
                        # magnitude histogram to inspect outliers
                        try:
                            mags = np.abs(concat)
                            plt.figure(figsize=(6,3))
                            plt.hist(mags, bins=200, log=True, color='gray')
                            plt.title("Symbol magnitude histogram (log scale)")
                            plt.xlabel("abs(symbol)"); plt.ylabel("count (log)")
                            plt.grid(True)
                            figs_shown += 1
                        except Exception:
                            pass
...
if __name__ == "__main__":
  ...
  if op == "T":
    ...
    if abs_corr.size == 0:
        ...
        for try_i in range(max_try):
            ...
            try:
                ...
                # --- safety checks and smoothing for Hk_est ---
                Hk_mag_raw = np.abs(Hk_est)
                # median-based flooring to avoid extremely small denominators
                if not np.all(np.isfinite(Hk_mag_raw)):
                    # invalid equalizer estimate, skip candidate
                    continue
                Hk_mag = medfilt(Hk_mag_raw, 5)
                # floor and cap to reasonable dynamic range
                floor_val = max(1e-3, np.percentile(Hk_mag, 5) * 0.2)
                cap_val = max(np.percentile(Hk_mag, 95) * 5.0, 1e3)
                Hk_mag = np.clip(Hk_mag, floor_val, cap_val)
                Hk_s = Hk_mag * np.exp(1j*np.angle(Hk_est))
                # quick diagnostic print (rate-limited)
                # verbose per-packet Hk diagnostics for the first LOG_HK_FIRST_PACKETS packets
                try:
                    p5 = np.percentile(Hk_mag, 5)
                    p50 = np.median(Hk_mag)
                    p95 = np.percentile(Hk_mag, 95)
                    # count how many packets we've logged using a global counter
                    if '_hk_verbose_logged' not in globals():
                        globals()['_hk_verbose_logged'] = 0
                    if globals()['_hk_verbose_logged'] < globals().get('LOG_HK_FIRST_PACKETS', 20):
                        globals()['_hk_verbose_logged'] += 1
                        print(f"[RX-HK-VDBG] pkt_log#{globals()['_hk_verbose_logged']} Hk_s stats: p5={p5:.3e} med={p50:.3e} p95={p95:.3e}")
                        # print a few per-subcarrier samples (first 8) to inspect phase/amp
                        try:
                            sample_vals = Hk_s[:8]
                            sample_str = ','.join(f"{np.abs(v):.3e}@{np.angle(v):+.3f}" for v in sample_vals)
                            print(f"[RX-HK-VDBG] Hk_s sample[0:8]: {sample_str}")
                        except Exception:
                            pass
                    # also limited status print tied to RS fail prints to avoid flooding
                    elif globals().get('_rs_fail_prints_count', 0) < 3:
                        print(f"[RX-DBG] Hk_s summary: p5={p5:.3e} med={p50:.3e} p95={p95:.3e}")
                except Exception:
                    pass
            except Exception:
                continue
            ...
            try:
                ...
                # changed check: treat as transmission header candidate if top nibble of first data byte != 0
    ...
    # Visualization: constellation + equalizer
    try:
        figs_shown = 0
