...
PREAMBLE_CORR_THRESHOLD_LIVE = 0.30 # для live поиска (нормированная корреляция)
PREAMBLE_CORR_THRESHOLD_REL = 0.5     # для file поиска: доля от max корреляции

# Поднесущие: установить так, чтобы было ровно 48 поднесущих в диапазоне начинающемся от ~300 Hz
k_low = int(math.ceil(320/ df))
REQUIRED_NSUB = 64
...
def live_receive_and_process():
    ...
    try:
        ...
        while not found_first:
            ...
            if peak_val < PREAMBLE_CORR_THRESHOLD_LIVE:
                time.sleep(0.02)
                continue
        ...
        try:
            ...
            for pkt_idx, pref in enumerate(expected_abs):
                ...
                loc_peak = int(np.argmax(norm_local))
                refined_abs = refine_center + loc_peak
                peak_refined = float(norm_local[loc_peak])
                peak_predicted = float(abs_corr[pref]) if (pref < len(abs_corr)) else 0.0
                # thresholds and decision parameters
                rel_threshold = 0.8
                snr_multiplier = 4.0
                max_displacement = SYMBOL_LEN // 2
                noise_floor = float(np.median(norm_local)) if norm_local.size > 0 else 0.0
                # choose refined if clearly better, else predicted if acceptable
                if peak_refined >= peak_predicted and peak_refined > max(rel_threshold * peak_predicted, noise_floor * snr_multiplier):
                    use_idx = int(refined_abs)
                    chosen_peak = peak_refined
                    chosen_mode = "refined"
                elif peak_predicted >= max(noise_floor * snr_multiplier, 1e-12):
                    use_idx = int(pref)
                    chosen_peak = peak_predicted
                    chosen_mode = "predicted"
                else:
                    # weak detection: pick larger peak but mark weak
                    if peak_refined >= peak_predicted:
                        use_idx = int(refined_abs)
                        chosen_peak = peak_refined
                        chosen_mode = "refined(weak)"
                    else:
                        use_idx = int(pref)
                        chosen_peak = peak_predicted
                        chosen_mode = "predicted(weak)"
                print(f"[LIVE] pkt{pkt_idx} predicted={pref} backoff={pref_backoff} refined={refined_abs} peak_refined={peak_refined:.3f} peak_pred={peak_predicted:.3f} -> using {chosen_mode} idx={use_idx} peak={chosen_peak:.3f}")

                # automatic schedule adjustment after consecutive large displacements
                if 'large_disp_count' not in globals():
                    globals()['large_disp_count'] = 0
                if abs(use_idx - pref) > max_displacement:
                    globals()['large_disp_count'] += 1
                else:
                    globals()['large_disp_count'] = 0
                if globals()['large_disp_count'] >= 3:
                    shift = use_idx - pref
                    base_abs = globals().get('sync_sample_abs', base_abs) + shift
                    expected_abs = [base_abs + int(x) for x in positions_no_preroll]
                    globals()['large_disp_count'] = 0
                    print(f"[SYNC-AUTO] adjusted base_abs by {shift}, new base_abs={base_abs}")

                globals()['rx'] = buf_now
                globals()['abs_corr'] = abs_corr
                decoded_bytes, rs_ok_count, used_preamble = decode_packet_at_candidate(use_idx, packet_blocks_val, packet_idx=pkt_idx, bytes_before_packet=bytes_collected, expected_total=expected_total)
...
if __name__ == "__main__":
  ...
  if op == "T":
    ...
    if abs_corr.size == 0:
        ...
        threshold = PREAMBLE_CORR_THRESHOLD_REL * peak_val if peak_val != 0 else 0.0
