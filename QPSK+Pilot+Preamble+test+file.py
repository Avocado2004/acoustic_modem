...
if __name__ == "__main__":
    ...
    if mode == "test":
        # простой тест: bits → QPSK → bits → текст
    ...
    wavfile.write("ofdm_tx.wav", fs,
                  (tx * np.iinfo(np.int16).max).astype(np.int16))
    print("Saved ofdm_tx.wav")
    ...
    # RX constellation with gradient
    plt.figure(figsize=(4,4))
    idxs = np.arange(len(rx_syms))
    plt.scatter(rx_syms.real,
                rx_syms.imag,
                c=idxs,
                cmap='RdBu',
                s=20,
