...
def ofdm_dem(rx, pre_cp):
    ...
    Демодуляция OFDM:
      - BPF + AGC
      - coarse timing по полной преамбуле с CP
      - оценка CFO по CP преамбулы
      - компенсация CFO
      - fine timing по преамбуле без CP
      - оценка канала и EQ
      - демап QPSK
    """
    # 1) BPF + AGC
    ...
    # 2) Coarse timing — корреляция с полной преамбулой (с CP)
    ...
    # 5) Оценка канала по преамбле (без CP)
    ...
    for i in range(n_blk):
        ...
        # фазовая правка по известным пилотам
