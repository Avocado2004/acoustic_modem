...
- 0: 1 байт - mode: b'T' или b'F' (или 0)
- 1: 1 байт - version/reserved (0)
- 2..9: 8 байт - data length (unsigned 64-bit, big-endian)
- 10..13: 4 байта - filename length (unsigned 32-bit, big-endian)
- 14..45: 32 байта - filename bytes (utf-8), padded/trimmed
- 46..55: 10 байт - reserved (zeros)
- 56..59: 4 байта - packet number (unsigned 32-bit, big-endian)
- 60..63: 4 байта - tail reserved (zeros)
"""
...
# -----------------------
def build_header(mode: bytes, data_len: int, filename_bytes: bytes = b'', packet_no: int = 0) -> bytes:
    """
    Собирает фиксированный 64-байтовый заголовок.
    mode: b'T' или b'F' (один байт)
    data_len: длина полезной нагрузки (unsigned 64-bit)
    filename_bytes: байты имени файла (utf-8). Будут усечены/дополнены до 32 байт в поле имени.
    packet_no: unsigned 32-bit
    """
    if mode not in (b'T', b'F'):
        mode = b'\x00'
    ver = b'\x00'
    data_len_packed = struct.pack('>Q', int(data_len) & 0xFFFFFFFFFFFFFFFF)
    name_len = len(filename_bytes) if filename_bytes is not None else 0
    name_len_packed = struct.pack('>I', int(name_len) & 0xFFFFFFFF)
    # name field fixed 32 bytes
    name_field = (filename_bytes[:32]).ljust(32, b'\x00')
    reserved_10 = b'\x00' * 10
    packet_packed = struct.pack('>I', int(packet_no) & 0xFFFFFFFF)
    tail_reserved = b'\x00' * 4
...
# -----------------------
# Визуализация созвездия (гарантированно доступна)
# -----------------------
...
if __name__ == "__main__":
    ...
    if op == "T":
        ...
        # номер пакета: по умолчанию 0 (можно расширить инкрементированием при фрагментации)
        packet_index = 0
        ...
        # assemble final output (silence + tx + silence)
        tx_out = np.concatenate((preroll, tx, postroll))

        # final normalization to int16 range (preserve relative noise level)
        max_abs = np.max(np.abs(tx_out))
        if max_abs == 0:
            scale_to_int16 = 1.0
        else:
            scale_to_int16 = 0.9 / max_abs

        tx_int16 = (tx_out * scale_to_int16 * np.iinfo(np.int16).max).astype(np.int16)
        wavfile.write("ofdm_acoustic_tx_with_noise.wav", fs, tx_int16)
        print("TX saved to ofdm_acoustic_tx_with_noise.wav (with controlled noise in silent regions)")
