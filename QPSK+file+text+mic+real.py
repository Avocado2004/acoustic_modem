...
if __name__ == "__main__":
    # 1) Выбор режима: File (по умолчанию) или Text
    ...
    if mode == "F":
        ...
        # 4) Кодирование имени и содержимого
        filename    = os.path.basename(file_path)
        name_bytes  = filename.encode("utf-8")
        if len(name_bytes) > 255:
            print("Имя файла слишком длинное (больше 255 байт).")
            sys.exit(1)

        file_data   = open(file_path, "rb").read()
        # 5) Заголовок: [8 байт BE size] [1 байт len(name)] [имя файла]
        header = (
            b"F" +                           # MARKER: файл
            struct.pack(">Q", file_size) +  # 8 байт BE size
        ...
        # Текстовый режим: передаём только размер и данные
   ...
   # 2) Восстанавливаем комплексный отклик с ограничением
    ...
    # 1) Прочитать 1-byte режима и поставить pos = 1
    mode_rx = all_bytes[0:1]
    pos     = 1

    # 2) Считать 8-байтный BE-size из offset pos и сдвинуть pos
    total_size = struct.unpack(">Q", all_bytes[pos:pos+8])[0]
    pos       += 8


    if mode_rx == b"F":
        # Файловый режим: следующий байт — длина имени
        name_len = all_bytes[pos]
        pos     += 1

        # Извлекаем имя
        name = all_bytes[pos:pos+name_len].decode("utf-8", errors="ignore")
        pos += name_len

        # Извлекаем содержимое точно в заявленном размере
        body = all_bytes[pos:pos+total_size]

        # Записываем файл с префиксом rx_
        out_path = "rx_" + name
        with open(out_path, "wb") as f:
            f.write(body)
        print(f"[RX FILE] Сохранён файл: {out_path}  (размер: {total_size} байт)")

    elif mode_rx == b"T":
        # Текстовый режим: просто извлекаем данные и печатаем
        text_bytes = all_bytes[pos:pos+total_size]
        rec_text   = text_bytes.decode("utf-8", errors="ignore")
        print("[RX TEXT]", rec_text)

    else:
        # если что-то пошло не так
        print("[RX ERROR] Неизвестный режим:", mode_rx)
    ...
    """ if mode == "T":
        # в текстовом режиме переменная text точно есть
        char_errs, cer = compute_text_errors(text, rec_text)
        print(f"[CER] Char errors: {char_errs}/{len(text)}  (CER = {cer:.2%})")
