...
# ширина окна поиска кандидата (в образцах) — полуширина окна вокруг pref_abs
...
def build_data_td(bits):
  # Use appropriate modulation mapping
  if MODULATION == "BPSK":
    syms = bpsk_map(bits)
  else:
    syms = qpsk_map(bits)
...
def build_header(mode: bytes, data_len: int, filename_bytes: bytes = b'', packet_no: int = 0, version: int = 0, packet_blocks: int = DEFAULT_PACKET_BLOCKS, crc32: int = 0) -> bytes:
    ...
    # set top 4 bits to 1 (0xF0), bits 3-2 for modulation, bits 1-0 for tx_type
    # Modulation: bits 3-2: 00=QPSK, 01=BPSK
...
def parse_header(header64: bytes):
    ...
    # Extract modulation from bits 3-2 of flags byte
    mod_bits = (b0 >> 2) & 0x03
...
def decode_packet_at_candidate(pref_abs, packet_blocks_expected, packet_idx=0, bytes_before_packet=0, expected_total=0):
    ...
    # --- новый детерминированный выбор кандидата: максимальная корреляция в фиксированном окне ---
    wlo = max(0, pref_abs - SYNC_WINDOW_HALF)
    whi = min(len(abs_corr) - 1, pref_abs + SYNC_WINDOW_HALF)
    if whi >= wlo:
        window = abs_corr[wlo:whi+1]
        if window.size > 0:
            # выбираем единственный кандидат — индекс максимума внутри окна
            rel_max = int(np.argmax(window))
            best_cand = wlo + rel_max
            combined = [best_cand]
        else:
            combined = []
    else:
    ...
    for cand in combined:
        ...
        for idxf, fr in enumerate(frames):
            ...
            # debug output: AGC per OFDM block
            try:
                if AGC_DEBUG:
                   print(f"[AGC] pkt={packet_idx} frame={idxf} cur_rms={cur_rms:.6e} est_rms={est_rms:.6e} gain_sym={gain_sym:.3f}")
            except Exception:
                pass
        ...
        if packet_idx == 0:
           ...
           # reconstruct merged 192-bytes header area by selecting for each
           # 8-byte chunk (codeword) the best copy among three header copies
            header_by_cw = bytearray(expected_hdr_len)
            # number of RS data-codewords per 64-byte header
            cw_per_hdr = 64 // RS_DATA_BYTES
            total_cw_for_3hdr = cw_per_hdr * 3
            # ensure decoded_blocks has at least that many entries (pad with zeros if necessary)
            if len(decoded_blocks) < total_cw_for_3hdr:
                for _p in range(len(decoded_blocks), total_cw_for_3hdr):
                    decoded_blocks.append((b'\x00'*RS_DATA_BYTES, False))
            # For each position i within a single 64-byte header (i=0..cw_per_hdr-1),
            # choose the first copy among copy0(i), copy1(i), copy2(i) that decoded_ok==True.
            for i in range(cw_per_hdr):
                chosen_msg = None
                # indices of codewords corresponding to this i in the three copies
                ...
                for ci in candidates_idx:
                    if ci < len(decoded_blocks):
                        ...
                        if ok_flag:
                            ...
                            break
                # if none decoded OK, pick first available (prefer copy0, then copy1, then copy2)
                ...
                # write chosen_msg into the corresponding place in the merged header
                start_b = i * RS_DATA_BYTES
                header_by_cw[start_b:start_b+RS_DATA_BYTES] = chosen_msg
            # Now replicate this selected 64-byte header across the three header slots in header_by_cw
            # because the logical header is 64 bytes; we built one 64-byte selection across i in 0..7.
            # Place the same selected 64-byte header into each of the three 64-byte copies sequentially.
            ...
            try:
                ...
                if 'Hk_smooth_list' in globals() and isinstance(globals()['Hk_smooth_list'], list) and len(globals()['Hk_smooth_list']) > 0:
                    ...
                    if len(Hlist) > 0 and PLOTTING_AVAILABLE:
                        ...
                        try:
                            plt.savefig("eq.png", dpi=150, bbox_inches="tight")
                        except Exception:
                            pass
                        plt.close()
                        figs_shown += 1
...
if __name__ == "__main__":
  ...
  if op == "T":
    ...
    transmission_header = build_header(b'F' if mode == "F" else b'T', total_data_len, filename_bytes=filename_bytes, packet_no=packet_no if 'packet_no' in locals() else 0, version=version, packet_blocks=packet_blocks, crc32=crc_val)
    ...
    # pkt0_decoded now should contain the 64-byte header in its first 64 bytes
    header64 = pkt0_decoded[:64]
    try:
      hdr = parse_header(header64)
    except Exception as e:
      ...
      print("[RX-ERR] aborting receive due to invalid transmission header")
      sys.exit(1)
    ...
    packet_blocks_from_hdr = hdr.get('packet_blocks', DEFAULT_PACKET_BLOCKS)
    if not isinstance(packet_blocks_from_hdr, int) or packet_blocks_from_hdr <= 0:
      print(f"[RX-ERR] invalid packet_blocks in header ({packet_blocks_from_hdr}), aborting")
      sys.exit(1)
    ...
    for pkt_idx, pref in enumerate(expected_abs):
        ...
        if pkt_idx == 0:
            if len(decoded_bytes) < 64:
                ...
                sys.exit(1)
            # payload starts after header coded region: 8 header cw each RS_HDR_CW_BYTES long
            payload_start = (64 // RS_DATA_BYTES) * RS_HDR_CW_BYTES
            if payload_start >= len(decoded_bytes):
                payload_chunk = b''
            else:
                payload_chunk = decoded_bytes[payload_start:]
    ...
    try:
        ...
        if 'Hk_smooth_list' in globals() and isinstance(globals()['Hk_smooth_list'], list) and len(globals()['Hk_smooth_list']) > 0:
            ...
            if len(Hlist) > 0 and PLOTTING_AVAILABLE:
                ...
                plt.title(f"Equalizer Gain vs Frequency (median of {Hstack.shape[0]} packets)")
                plt.xlabel("Frequency (Hz)"); plt.ylabel("Gain"); plt.grid(True); plt.legend()
                # explicitly save to eq.png (avoid any constructed/empty filename)
                try:
                    plt.savefig("eq.png", dpi=150, bbox_inches="tight")
                except Exception:
                    pass
