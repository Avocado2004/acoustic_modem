...
# -----------------------
# параметры ACE (подберите экспериментально)
ACE_MAX_ITERS = 5          # количество итераций ACE на символ
ACE_PEAK_THRESHOLD = 1.01   # порог в относительных единицах над средней амплитудой временного блока
ACE_STEP = 0.3           # максимальная доля коррекции на итерацию (0..1)
ACE_ALLOW_EXPANSION = True # разрешать только расширение точки (для QPSK)

def ace_reduce_peaks(ds, Nfft_local, subc_inds_local):
  """
  Вход:
    ds - complex array length Nsub (поднесущие, перед применением subc_phases)
    Nfft_local - FFT size (обычно global Nfft)
    subc_inds_local - indices поднесущих в частотной сетке
  Возврат:
    ds_mod - скорректированные поднесущие (same shape)
  Принцип:
    1) формируем частотный вектор X, вставляем ds в subc_inds и зеркально
    2) IFFT -> time domain x
    3) находим пики |x| > threshold*mean(|x|)
    4) если пики есть, формируем коррекцию в частотной области, проецируем её обратно только на поднесущие данных,
       и обновляем ds, но для QPSK разрешаем только увеличение амплитуды символов (radial expansion)
    5) повторяем итеративно
  """
  try:
    if ds is None or len(ds) == 0:
      return ds
    # prepare frequency-domain vector for this symbol
    ...
    for it in range(ACE_MAX_ITERS):
      x_td = np.fft.ifft(X)
      x_td_real = np.real(x_td)  # we transmit real-valued time signal
      ...
      if peak_idx.size == 0:
        break
      # build a time-domain correction that reduces peaks (clipping-like residual)
      corr_td = np.zeros_like(x_td, dtype=complex)
      # for each peak sample, compute small negative correction proportional to exceedance
      exceed = mag[peak_idx] - thresh
      # sign of real part to preserve phase sign (time signal is real)
      corr_td[peak_idx] = - (exceed / (mag[peak_idx] + 1e-12)) * x_td[peak_idx] * ACE_STEP
      # transform correction to freq domain
      Corr_fd = np.fft.fft(corr_td)
      # project correction onto data subcarriers: keep only subc_inds and -subc_inds
      Corr_proj = np.zeros_like(Corr_fd)
      Corr_proj[subc_inds_local] = Corr_fd[subc_inds_local]
      Corr_proj[-subc_inds_local] = Corr_fd[-subc_inds_local]
      # apply small correction to X (move opposite to peak direction)
      X_new = X + Corr_proj
      # extract updated ds candidate
      ds_cand = X_new[subc_inds_local].copy()
      # for QPSK keep symbol angle same, allow radial expansion only if permitted
      if ACE_ALLOW_EXPANSION:
        # enforce phase preservation and allow amplitude growth up to factor (1+ACE_STEP)
        phases = np.angle(ds)
        mags_old = np.abs(ds)
        mags_cand = np.abs(ds_cand)
        # prevent amplitude reduction: only allow mags_cand >= mags_old; if less, clip to mags_old
        mags_new = np.maximum(mags_old, mags_cand)
        # optionally limit per-iteration growth
        mags_new = np.minimum(mags_new, mags_old * (1.0 + ACE_STEP))
        ds = mags_new * np.exp(1j * phases)
        # rebuild X from ds for next iteration
        X = np.zeros(Nfft_local, dtype=complex)
        X[subc_inds_local] = ds
        X[-subc_inds_local] = np.conj(ds)
      else:
        # unrestricted accept candidate
        ds = ds_cand
        X = X_new
    return ds
  except Exception:
...
def ofdm_symbol(data_syms):
  ...
  # --- per-symbol amplitude normalization (TX) ---
  # нормируем комплексные поднесущие по их среднеквадратичному уровню
  # чтобы каждый OFDM-символ имел стабильную целевую амплитуду SYMBOL_TX_TARGET
  try:
    eps = 1e-12
...
# - добавлено: при RS failure после синхронизации печатается частота поднесущей, на которой произошло падение.
# - декодер обновляет globals()['last_packet_end_sample'] и globals()['last_packet_used_pre'] для корректного определения конца полезных данных.
# -----------------------
...
def decode_packet_at_candidate(pref_abs, packet_blocks_expected, packet_idx=0, bytes_before_packet=0, expected_total=0):
    ...
    for cand in combined:
        try:
            ...
            # correct FOE in pilots before channel estimation
            ...
            # phase estimation using ZC (global common phase offset)
            phi_est = 0.0
            if S_zc_fd is not None:
                try:
                    ...
                    phi_est = 0.0
            # apply phi_est compensation to pilots (remove common phase)
            if phi_est != 0.0:
        ...
        try:
            # взять участок преамбулы из rx и оценить RMS
        ...
        # применим packet_gain к соответствующему сегменту пакета, когда будем извлекать seg
        # далее при извлечении seg: seg = packet_gain * rx[pkt_data_start : pkt_data_start + pkt_payload_samples]
        # замените оригинальную строку извлечения seg на версию с packet_gain
        ...
        for idxf, fr in enumerate(frames):
            ...
            # --- SYMBOL-LEVEL (frame) AGC (insert inside frame loop, just before F = np.fft.fft(useful_corr)) ---
            

            # useful = fr[Ncp:] уже вычислен; теперь оценка rms и скользящая нормировка
            ...
            # масштабируем комплексный useful, не трогая фазу
            useful = useful * gain_sym
            # затем выполняем коррекцию частотной ошибки и FFT как раньше
        ...
        for ci in range(n_cw):
            ...
            try:
                ...
                if do_print:
                    ...
                    # compute corresponding subcarrier frequency for this cw (symbols -> subcarrier)
                    try:
                    ...
                    if packet_idx == 0:
                        ...
                        # print frequency only if we are already in post-sync mode
        ...
        if rs_ok > best_result[1]:
            ...
            try:
                ...
                # store last packet end sample (for plotting range)
...
# - дополнения: после успешного декодирования первого пакета помечаем post_sync и запоминаем sync_sample_abs.
# - визуализация ограничена [sync_sample_abs : last_packet_end_sample]
# -----------------------
def live_receive_and_process():
    ...
    try:
        ...
        try:
            ...
            # mark post-sync and store sync sample absolute index for plotting range
            ...
            # plotting: ограничиваем срез от sync_sample_abs до last_packet_end_sample
            try:
                ...
                if plot_slice is not None:
                    # regenerate abs equalizer/constellation from stored rx_syms_list but only keep symbols collected after sync
                    if 'rx_syms_list' in globals() and isinstance(globals()['rx_syms_list'], list) and len(globals()['rx_syms_list']) > 0:
                        ...
                        # fallback: plot waveform slice as simple time plot to confirm range
...
if __name__ == "__main__":
  ...
  if op == "T":
    ...
    # set post_sync and sync sample for plotting
    globals()['post_sync'] = True
    ...
    # Visualization: constellation + equalizer, limited to post-sync useful range
    try:
        sync_start = globals().get('sync_sample_abs', None)
        last_end = globals().get('last_packet_end_sample', None)
        if sync_start is None:
        ...
        if plot_slice is not None:
            if 'rx_syms_list' in globals() and isinstance(globals()['rx_syms_list'], list) and len(globals()['rx_syms_list']) > 0:
                ...
                # fallback time-domain plot of the focused slice
                plt.figure(figsize=(8,3))
                tvec = np.arange(len(plot_slice)) / float(fs)
                plt.plot(tvec, plot_slice)
                plt.title("Time-domain slice (post-sync)")
