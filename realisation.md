...
**Пилот-поднесущая:** Поднесущая с индексом 24 (25-я по счёту) зарезервирована под пилот-сигнал для будущего функционала. В преамбуле она не используется (тишина), в данных постоянно передаётся символ "0" (QPSK: (1+1j)/√2, BPSK: +1). В приёмнике эта поднесущая игнорируется при демодуляции.
...
#### OFDM:
- `ofdm_symbol(data_syms, return_fd=False, is_preamble=False)` — сборка OFDM символа: нормализация → ACE → фазы → IFFT → CP. Параметр `is_preamble` управляет поведением пилот-поднесущей: в преамбуле = 0, в данных = символ "0".
- `build_data_td(bits, collect_fd=False)` — модуляция потока битов в сигнал во временной области. Данные размещаются на 48 поднесущих, пилот-поднесущая заполняется автоматически.
...
- В каждом OFDM символе данных поднесущая с индексом `PILOT_SUBC_INDEX` содержит символ "0":
  - QPSK: (1+1j)/√2
  - BPSK: +1
- ACE не изменяет значение пилот-поднесущей

#### AdaptiveEqualizer — адаптивный эквалайзер:
- `__init__(initial_Hk, alpha=0.02, modulation)` — инициализация оценкой канала из преамбулы. Hk размер = Nsub (49).
